...
type LearningModule = { id: string; title: string; category: "Foundation" | "Algorithm" | "Hybrid"; difficulty: string; minutes: number; prereqs: string[]; summary: string; concept: string; math: string; why: string; steps: string[]; keyPoints: string[]; quiz: { question: string; options: string[]; answer: number; explanation: string } };
const learningModules: LearningModule[] = [
  { id: "qubits", title: "Qubits", category: "Foundation", difficulty: "Beginner", minutes: 18, prereqs: [], summary: "Represent quantum information using basis states, amplitudes and the Bloch sphere.", concept: "A qubit is a normalized state in a two-dimensional complex vector space. Unlike a classical bit, its state is described by amplitudes; only measurement produces a classical 0 or 1.", math: "|ψ⟩ = α|0⟩ + β|1⟩,   |α|² + |β|² = 1", why: "Qubits are the basic information units manipulated by every quantum circuit.", steps: ["Identify |0⟩ and |1⟩ as computational basis states.", "Interpret squared amplitude magnitude as measurement probability.", "Use the Bloch sphere for pure single-qubit states; remember global phase is not observable."], keyPoints: ["Amplitudes are complex numbers, not probabilities.", "Normalization makes all outcome probabilities sum to one.", "A qubit is not simply a bit that is both values at once."], quiz: { question: "If α = 1/√2 and β = i/√2, what is P(1)?", options: ["0", "1/2", "i/2", "1"], answer: 1, explanation: "P(1)=|β|²=|i/√2|²=1/2." } },
  { id: "superposition", title: "Superposition", category: "Foundation", difficulty: "Beginner", minutes: 20, prereqs: ["qubits"], summary: "Understand coherent combinations of basis states and their interference.", concept: "Superposition means a quantum state is a linear combination of basis states. Its amplitudes can reinforce or cancel during later operations, which is why quantum algorithms can exploit interference.", math: "H|0⟩ = (|0⟩ + |1⟩)/√2", why: "It lets an algorithm transform amplitudes across many basis states before measurement selects one outcome.", steps: ["Start in a known basis state such as |0⟩.", "Apply H to create equal-magnitude amplitudes.", "Apply another H and observe destructive interference return the state to |0⟩."], keyPoints: ["Measurement samples one basis outcome.", "Relative phase affects interference.", "Superposition alone does not make every computation exponentially faster."], quiz: { question: "What does a second H gate do to H|0⟩?", options: ["Returns |0⟩", "Produces |1⟩", "Measures the qubit", "Creates entanglement"], answer: 0, explanation: "H is self-inverse: H²=I, so H(H|0⟩)=|0⟩." } },
  { id: "measurement", title: "Measurement", category: "Foundation", difficulty: "Beginner", minutes: 18, prereqs: ["qubits", "superposition"], summary: "Convert quantum amplitudes into classical outcomes using the Born rule.", concept: "A computational-basis measurement returns a bit string with probability equal to the squared magnitude of its amplitude. After an ideal projective measurement, the state is consistent with the observed outcome.", math: "P(x) = |⟨x|ψ⟩|²", why: "Quantum processors must eventually communicate useful results to classical systems.", steps: ["Choose a measurement basis.", "Calculate outcome probabilities with the Born rule.", "Repeat shots to estimate the distribution; finite samples introduce statistical error."], keyPoints: ["Measurement is generally non-unitary and irreversible.", "Shots estimate probabilities; they do not reveal an unknown amplitude directly.", "Changing the basis before measurement reveals different information."], quiz: { question: "A state has amplitude −i/2 for |1⟩. What is its contribution to P(1)?", options: ["−1/2", "1/4", "i/4", "1/2"], answer: 1, explanation: "Probability uses magnitude squared: |−i/2|²=1/4." } },
  { id: "gates", title: "Quantum Gates", category: "Foundation", difficulty: "Beginner", minutes: 30, prereqs: ["qubits"], summary: "Use unitary single- and multi-qubit operations to transform states.", concept: "Quantum gates are reversible unitary transformations. X flips computational basis states, Z changes phase, H mixes basis states, rotations move around Bloch-sphere axes, and controlled gates condition one operation on another qubit.", math: "U†U = I;   X² = H² = Z² = I", why: "Algorithms are compiled into ordered gate sequences that hardware can execute.", steps: ["Use X for a bit flip and Z for a phase flip.", "Use RX, RY and RZ for parameterized rotations.", "Use CX or CZ to create correlations and, with superposition, entanglement."], keyPoints: ["Gate order matters because many operators do not commute.", "Unitary gates preserve total probability.", "Measurement is an operation, but not a reversible unitary gate."], quiz: { question: "Which gate adds a phase of −1 to |1⟩ while leaving |0⟩ unchanged?", options: ["X", "Z", "H", "CX"], answer: 1, explanation: "Z|0⟩=|0⟩ and Z|1⟩=−|1⟩." } },
  { id: "entanglement", title: "Entanglement", category: "Foundation", difficulty: "Intermediate", minutes: 28, prereqs: ["superposition", "gates"], summary: "Create and interpret correlations that cannot be factored into individual qubit states.", concept: "A multi-qubit state is entangled when it cannot be written as a tensor product of separate states for each subsystem. Bell states produce correlations stronger than classical local models allow.", math: "|Φ⁺⟩ = (|00⟩ + |11⟩)/√2", why: "Entanglement powers teleportation, superdense coding, device tests and many quantum protocols.", steps: ["Apply H to q[0] to create two coherent branches.", "Apply CX with q[0] controlling q[1].", "Measure both: ideal outcomes are 00 and 11 with equal probability."], keyPoints: ["Correlations do not permit faster-than-light communication.", "Each qubit alone can look random while the pair has definite correlations.", "Entanglement is sensitive to noise and unwanted environmental interaction."], quiz: { question: "Which circuit prepares |Φ⁺⟩ from |00⟩?", options: ["X on both qubits", "H on q0, then CX q0→q1", "CX then H on q1", "Measure then H"], answer: 1, explanation: "H creates the branches and CX correlates the target with the control." } },
  { id: "circuits", title: "Quantum Circuits", category: "Foundation", difficulty: "Intermediate", minutes: 25, prereqs: ["gates", "measurement"], summary: "Read circuit diagrams, track time order and calculate depth.", concept: "A quantum circuit is an ordered computation: wires represent qubits, gates represent operations and measurements produce classical data. Gates in the same column can be parallel only when their qubit resources do not conflict.", math: "|ψout⟩ = Un ··· U2 U1 |ψin⟩", why: "Circuit width, gate count and depth determine resource demand and strongly influence hardware error.", steps: ["Read operations from left to right.", "Separate sequential dependencies from parallel operations.", "Track control-target relationships and terminal measurements."], keyPoints: ["Matrix multiplication acts right-to-left in the state equation.", "Depth counts dependent layers, not just gates.", "Equivalent circuits may have very different hardware cost."], quiz: { question: "Why can two gates sometimes share a circuit layer?", options: ["They use the same qubit", "They act on disjoint qubits without dependency", "They are both measurements", "Every pair of gates can"], answer: 1, explanation: "Operations on disjoint qubits can often execute in parallel." } },
  { id: "deutsch-jozsa", title: "Deutsch–Jozsa", category: "Algorithm", difficulty: "Intermediate", minutes: 32, prereqs: ["superposition", "circuits"], summary: "Distinguish a promised constant Boolean function from a balanced one.", concept: "Deutsch–Jozsa solves a promise problem: the oracle function is guaranteed constant or balanced. Phase kickback and interference make one ideal oracle query sufficient to distinguish the two cases exactly.", math: "Uf|x,y⟩ = |x, y ⊕ f(x)⟩", why: "It clearly demonstrates how interference can reduce query complexity, though the promise problem itself is mainly educational.", steps: ["Prepare input qubits in |0⟩ and an ancilla in |1⟩.", "Apply H gates, query the oracle once, then apply H to inputs.", "Measure inputs: all zeros means constant; any nonzero result means balanced in the ideal circuit."], keyPoints: ["The speedup is in oracle queries under a strict promise.", "The oracle must be implemented as a reversible unitary.", "Noise can blur the ideal all-zero decision pattern."], quiz: { question: "What ideal measurement indicates a constant function?", options: ["All input bits are 0", "All input bits are 1", "The ancilla is 1", "Any random string"], answer: 0, explanation: "After interference, a constant oracle yields the all-zero input register." } },
  { id: "grover", title: "Grover’s Algorithm", category: "Algorithm", difficulty: "Intermediate", minutes: 38, prereqs: ["superposition", "circuits"], summary: "Amplify the amplitude of marked items in an unstructured search space.", concept: "Grover search alternates a phase-marking oracle with a diffusion operator. Each iteration rotates the state toward the marked subspace, giving quadratic query improvement over classical unstructured search.", math: "Queries ≈ (π/4)√(N/M)", why: "Amplitude amplification is a reusable primitive for search and probabilistic quantum subroutines.", steps: ["Prepare a uniform superposition over N candidates.", "Mark solutions by changing their phase.", "Reflect amplitudes about their mean and repeat the near-optimal number of times before measuring."], keyPoints: ["The advantage is quadratic, not exponential.", "Too many iterations rotate past the solution and reduce success probability.", "Oracle construction cost must be included in real resource estimates."], quiz: { question: "For one marked item among N, Grover needs roughly how many oracle queries?", options: ["N", "log₂N", "√N", "N²"], answer: 2, explanation: "Amplitude amplification finds a marked item using O(√N) oracle queries." } },
  { id: "teleportation", title: "Quantum Teleportation", category: "Algorithm", difficulty: "Intermediate", minutes: 34, prereqs: ["entanglement", "measurement"], summary: "Transfer an unknown qubit state using entanglement and two classical bits.", concept: "Teleportation consumes a shared Bell pair. Alice performs a Bell-basis measurement and sends two classical bits; Bob uses them to choose corrections that reconstruct the original state.", math: "1 shared ebit + 2 classical bits → transfer of 1 qubit state", why: "It is fundamental to quantum networks, repeaters and distributed quantum computation.", steps: ["Share an entangled pair between Alice and Bob.", "Alice entangles the message qubit with her half and measures both.", "Bob applies X and/or Z corrections determined by the two classical bits."], keyPoints: ["The original state is destroyed by Alice’s measurement.", "No matter or usable information travels faster than light.", "Teleportation transfers a state, not the physical particle."], quiz: { question: "What must Alice send Bob after her measurement?", options: ["The original qubit", "Two classical bits", "One photon only", "Nothing"], answer: 1, explanation: "Bob needs two classical measurement bits to select the correction." } },
  { id: "qft", title: "Quantum Fourier Transform", category: "Algorithm", difficulty: "Advanced", minutes: 42, prereqs: ["gates", "circuits"], summary: "Encode periodic structure into phases using a quantum basis transformation.", concept: "QFT is the unitary analogue of the discrete Fourier transform on amplitudes. It is usually valuable as a subroutine—especially inverse QFT in phase estimation—not as a way to read every Fourier coefficient.", math: "QFT|x⟩ = 1/√N Σᵧ e^(2πixy/N)|y⟩", why: "It exposes periodicity and phase information used by phase estimation and Shor-type algorithms.", steps: ["Apply H to separate a qubit into phase-sensitive branches.", "Apply controlled phase rotations with decreasing angles.", "Reverse qubit order with swaps, or account for reversed output convention."], keyPoints: ["QFT uses O(n²) standard gates, with useful approximations possible.", "Measurement cannot reveal all N amplitudes efficiently.", "Controlled rotations become increasingly small and noise-sensitive."], quiz: { question: "Where is inverse QFT commonly used?", options: ["Phase estimation", "Classical sorting", "Error-free cloning", "Direct amplitude printing"], answer: 0, explanation: "Inverse QFT converts encoded eigenphases into measurable bit strings." } },
  { id: "vqe-qaoa", title: "VQE and QAOA", category: "Hybrid", difficulty: "Advanced", minutes: 45, prereqs: ["circuits", "measurement"], summary: "Build hybrid variational loops for chemistry and combinatorial optimisation.", concept: "VQE estimates low-energy states of a Hamiltonian; QAOA alternates cost and mixer evolutions for optimisation. Both use a parameterized quantum circuit evaluated by measurements and a classical optimizer that updates parameters.", math: "θ* = arg minθ ⟨ψ(θ)|H|ψ(θ)⟩", why: "Variational methods trade deep fault-tolerant circuits for shallower repeated experiments, making them important research tools for noisy devices.", steps: ["Choose an ansatz and measurable objective Hamiltonian.", "Run circuits to estimate expectation values.", "Use a classical optimizer to update parameters until a stopping criterion is reached."], keyPoints: ["Results depend on ansatz, optimizer, shots and noise.", "Barren plateaus can make gradients extremely small.", "Practical quantum advantage is not guaranteed and must be benchmarked against strong classical methods."], quiz: { question: "What does the classical optimizer change in a variational algorithm?", options: ["Hardware temperature", "Circuit parameters", "Number of qubits in the chip", "Measurement laws"], answer: 1, explanation: "It updates parameterized gate angles to improve the measured objective." } },
];
...
function starterCircuit(id: string): Circuit {
  ...
  const presets: Record<string, Array<[GateName, number, number?]>> = { qubits: [["X", 0]], superposition: [["H", 0]], measurement: [["H", 0], ["M", 0]], gates: [["H", 0], ["X", 1], ["Z", 0]], entanglement: [["H", 0], ["CX", 0, 1], ["M", 0], ["M", 1]], circuits: [["H", 0], ["X", 1], ["CX", 0, 1], ["M", 0], ["M", 1]], "deutsch-jozsa": [["X", 1], ["H", 0], ["H", 1], ["CX", 0, 1], ["H", 0], ["M", 0]], grover: [["H", 0], ["H", 1], ["CZ", 0, 1], ["H", 0], ["X", 0], ["M", 0], ["M", 1]], teleportation: [["H", 1], ["CX", 1, 2], ["CX", 0, 1], ["H", 0], ["M", 0], ["M", 1]], qft: [["H", 0], ["S", 1], ["H", 1]], "vqe-qaoa": [["RY", 0], ["RY", 1], ["CX", 0, 1], ["RZ", 1]] };
      ...
      <div className="grid gap-5 p-5 lg:grid-cols-[.8fr_1.2fr]">
        <div>
          ...
          {status !== "initial" && (
            <Progress
              value={progress}
              className="mt-4 bg-white/10 [&>div]:bg-gradient-to-r [&>div]:from-primary [&>div]:to-secondary"
            />
          )}
        </div>
...
function RouteOverview({ page, openLesson, tryModule, navigate }: { page: "algorithms" | "projects" | "challenges" | "profile"; openLesson: (id: string) => void; tryModule: (id: string) => void; navigate: (page: Page) => void }) {
  if (page === "algorithms") return <div className="page-enter mx-auto max-w-[1500px] p-5 sm:p-8"><p className="text-sm font-semibold uppercase tracking-[.16em] text-secondary">Algorithm library</p><h1 className="glow-text mt-2 text-4xl font-bold sm:text-5xl">From theory to runnable circuits</h1><p className="mt-3 max-w-3xl leading-7 text-muted-foreground">Study the idea, resource trade-offs and circuit structure behind foundational quantum algorithms, then load a deterministic teaching circuit in the Composer.</p><div className="mt-8 grid gap-4 md:grid-cols-2 xl:grid-cols-3">{learningModules.filter(m => m.category !== "Foundation").map(module => <article key={module.id} className="algorithm-card"><div className="flex items-center justify-between"><span className="text-xs uppercase tracking-wider text-secondary">{module.category}</span><span className="text-xs text-muted-foreground">{module.minutes} min</span></div><h2 className="mt-4 text-xl font-semibold">{module.title}</h2><p className="mt-3 text-sm leading-6 text-muted-foreground">{module.summary}</p><p className="mt-4 rounded-lg border border-primary/15 bg-primary/5 p-3 font-mono text-xs text-primary">{module.math}</p><div className="mt-5 grid grid-cols-2 gap-2"><button onClick={() => openLesson(module.id)} className="rounded-lg border border-border p-2 text-sm">Study</button><button onClick={() => tryModule(module.id)} className="rounded-lg bg-secondary p-2 text-sm font-semibold text-secondary-foreground">Try circuit</button></div></article>)}</div></div>;
  if (page === "challenges") return <div className="page-enter mx-auto max-w-[1200px] p-5 sm:p-8"><p className="text-sm font-semibold uppercase tracking-[.16em] text-secondary">Practice arena</p><h1 className="glow-text mt-2 text-4xl font-bold sm:text-5xl">Circuit challenges</h1><p className="mt-3 text-muted-foreground">Turn concepts into small, verifiable circuit-building goals.</p><div className="mt-8 grid gap-4 md:grid-cols-3">{[["Bell-state builder", "Prepare equal 00 and 11 probabilities.", "entanglement"], ["Gate cancellation", "Find and remove a self-inverse gate pair.", "gates"], ["Grover marker", "Inspect amplitude amplification for |11⟩.", "grover"]].map(([title, copy, id], i) => <article key={title} className="glass rounded-2xl p-6"><span className="text-xs text-secondary">Challenge {i + 1}</span><h2 className="mt-4 text-xl font-semibold">{title}</h2><p className="mt-3 min-h-16 text-sm leading-6 text-muted-foreground">{copy}</p><button onClick={() => tryModule(id)} className="mt-5 w-full rounded-xl border border-secondary/40 p-3 text-sm text-secondary">Load challenge circuit</button></article>)}</div></div>;
  return <div className="page-enter mx-auto max-w-[1100px] p-5 sm:p-8"><p className="text-sm font-semibold uppercase tracking-[.16em] text-secondary">Learner profile</p><h1 className="glow-text mt-2 text-4xl font-bold sm:text-5xl">Your quantum journey</h1><div className="mt-8 grid gap-4 md:grid-cols-3"><article className="glass rounded-2xl p-6"><Zap className="size-5 text-primary" /><p className="mt-5 text-4xl font-bold">4,500</p><p className="mt-2 text-sm text-muted-foreground">Learning XP</p></article><article className="glass rounded-2xl p-6"><Flame className="size-5 text-secondary" /><p className="mt-5 text-4xl font-bold">12 days</p><p className="mt-2 text-sm text-muted-foreground">Current streak</p></article><article className="glass rounded-2xl p-6"><GraduationCap className="size-5 text-primary" /><p className="mt-5 text-4xl font-bold">5</p><p className="mt-2 text-sm text-muted-foreground">Modules in progress</p></article></div><article className="glass mt-4 rounded-2xl p-6"><h2 className="text-xl font-semibold">Achievement badges</h2><div className="mt-5 flex flex-wrap gap-3">{["Qubit Explorer", "Bell Builder", "Three-day Streak", "Code Translator"].map(item => <span key={item} className="rounded-full border border-primary/25 bg-primary/8 px-4 py-2 text-sm text-primary">{item}</span>)}</div><button onClick={() => navigate("learning")} className="mt-6 rounded-xl bg-secondary px-5 py-3 font-semibold text-secondary-foreground">Continue learning</button></article></div>;
}
