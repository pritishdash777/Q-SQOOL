...
function Dashboard({ navigate }: { navigate: (page: Page) => void }) {
  ...
  return <div className="page-enter mx-auto max-w-[1500px] p-5 sm:p-8">
    ...
    <div className="mt-8 grid gap-4 lg:grid-cols-[1fr_1fr_1.2fr_1.4fr]">
      ...
      <article className="glass hover-rise rounded-2xl p-5"><div className="flex justify-between text-sm text-muted-foreground"><span>Learning streak</span><Flame className="size-4 text-secondary" /></div><p className="mt-5 text-4xl font-bold">0 <span className="text-base font-normal text-muted-foreground">days</span></p><div className="mt-5 flex gap-2">{[1, 1, 1, 1, 0].map((v, i) => <span key={i} className={`h-1.5 flex-1 rounded-full ${v ? "bg-secondary shadow-[0_0_10px_#43e7ff]" : "bg-white/10"}`} />)}</div></article>
      <article className="glass hover-rise relative overflow-hidden rounded-2xl p-5"><p className="text-sm text-muted-foreground">Skill mastery</p><div className="relative mx-auto mt-3 grid size-28 place-items-center"><div className="absolute inset-1 animate-[spin-slow_14s_linear_infinite] rounded-full border border-dashed border-secondary/40" /><div className="absolute inset-5 rounded-full border border-primary/40" /><span className="text-2xl font-bold">0%</span></div></article>
