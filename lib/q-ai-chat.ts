...
export type ChatAnswer = { text: string; truncated: boolean };
...
export async function requestChat(messages: ChatTurn[], page: string, signal: AbortSignal): Promise<ChatAnswer> {
  const timeout = AbortSignal.timeout(55000);
  try {
    const response = await fetch("/api/q-ai", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ messages: chatHistory(messages), page }),
